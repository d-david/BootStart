...
// Create Custom Post Type
    ...
    add_action('admin_menu', 'dd_add_slidelink_2_meta_box');
    function dd_add_slidelink_2_meta_box() {
     
        global $slidelink_2_metabox;        
     
        foreach($slidelink_2_metabox['page'] as $page) {
            add_meta_box($slidelink_2_metabox['id'], $slidelink_2_metabox['title'], 'dd_show_slidelink_2_box', $page, 'normal', 'default', $slidelink_2_metabox);
        }
    }
     
    // function to show meta boxes
    function dd_show_slidelink_2_box()  {
        global $post;
        global $slidelink_2_metabox;
        global $dd_prefix;
        global $wp_version;
        ...
        echo '<input type="hidden" name="dd_slidelink_2_meta_box_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
        ...
        foreach ($slidelink_2_metabox['fields'] as $field) {
            ...
            echo '<tr>',
                    ...
                    '<td class="dd_field_type_' . str_replace(' ', '_', $field['type']) . '">';
            switch ($field['type']) {
                case 'text':
    ...
    add_action('save_post', 'dd_slidelink_2_save');
    function dd_slidelink_2_save($post_id) {
        global $post;
        global $slidelink_2_metabox;
         
        // verify nonce
        if (!wp_verify_nonce($_POST['dd_slidelink_2_meta_box_nonce'], basename(__FILE__))) {
            return $post_id;
        }
